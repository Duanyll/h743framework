...
#define WRITE(name, val)                                        \
    HAL_GPIO_WritePin(DDS_##name##_GPIO_Port, DDS_##name##_Pin, \
                      (val) ? HIGH : LOW)
...
void AD9959_SetSDIO0Input() {
    ...
    HAL_GPIO_Init(DDS_SDIO0_GPIO_Port, &GPIO_InitStruct);
}
...
uint8_t AD9959_ReadRaw() {
    ...
    for (int i = 7; i >= 0; i--) {
        WRITE(CLK, LOW);
        AD9959_Delay();
        data |= HAL_GPIO_ReadPin(DDS_SDIO0_GPIO_Port, DDS_SDIO0_Pin) << i;
        WRITE(CLK, HIGH);
        AD9959_Delay();
    }
    AD9959_Delay();

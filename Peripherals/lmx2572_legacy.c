...
void LMX2572L_Init(LMX2572L_Pins *p) {
  pins = p;
  GPIO_InitTypeDef s = {
...
输出：无
*******************************/
void LMX2572L_RegisterData(double Freqout, double Space, uint8_t power) {
...
void LMX2572L_SetFrequency(uint32_t freq) {
  ...
  Chanspace = 1;       // KHz
  POW = 50;
  LMX2572L_RegisterData(Frequencyout, Chanspace, POW);
  LMX2572L_SendData(0x00241E);
  LMX2572L_SendData(0x00241C);
  LMX2572L_SendDataArray(R, 113);
  LMX2572L_SendData(R[0]);
